...
- **`fetch_lineup` 첫 호출**: 팀 스탯 4개 페이지 수집은 `fetch_multiple_html()` 병렬화로 개선됨.
  - 측정: 순차 12.52s → 병렬 3.53s
  - `max_concurrency=4`로 동시성 제한 적용
- **앱 초기 로드**: `load_games()`에서 선수 기록 테이블 사전 로드를 제거해 경기 목록을 우선 표시하도록 변경함. 상세 데이터는 경기 선택 시 점진 로딩함.
